...
class TestPydanticPIIProtector:
    ...
    def test_user_profile_to_summary(self):
        ...
        protected_summary = UserSummary(
            ...
            tags=["engineer", "consultant", protected_profile.full_name.split()[0]]  # Using first name as tag
        )
        
        # Unprotect output
        unprotected_summary, issues = protector.unprotect_model(protected_summary, state)
        ...
        assert "Robert" in unprotected_summary.profile_summary or "[NAME_FIRST_" in protected_profile.bio
        assert any("Robert" in tag or "[NAME_FIRST_" in tag for tag in unprotected_summary.tags)
        assert len(issues) == 0
    ...
    def test_customer_data_to_order(self):
        ...
        assert "456 Oak Ave" not in str(unprotected_order)  # Billing address not transferred
        assert len(issues) == 0
    ...
    def test_field_annotations_with_transformation(self):
        ...
        assert "Carol is a developer" in unprotected_analysis.recommendations[2]  # Bio was not redacted
        assert len(issues) == 0
...
class TestPIIConfigDecorator:
    ...
    def test_config_without_detector_raises(self):
        """Test that PIIConfig without detector raises error."""
        with pytest.raises(ValueError, match="detector must be provided"):
            config = PIIConfig(detector=None)
            
            @config.protect
            def bad_function(user: SimpleUser) -> SimpleUser:
                return user
            
            # Try to call it
            bad_function(SimpleUser(name="Test", email="test@example.com"))
    ...
    def test_mixed_model_transformation(self):
        ...
        def process_order_with_metadata(
            ...
            assert "[NAME_" in customer.customer_name or "[PERSON_" in customer.customer_name
    ...
    def test_multiple_different_model_arguments(self):
        ...
        config = PIIConfig(detector=detector, batch_detection=True, use_name_parsing=False)
...
class TestNameComponentParsing:
    ...
    def test_name_component_detection(self):
        ...
        # Entity tracker assigns indices alphabetically: "Dr." -> 1, "Johnson" -> 2, "Sarah" -> 3
        assert protected.name == "[NAME_TITLE_1] [NAME_FIRST_3] [NAME_LAST_2]"
        
        # Verify state has all component tokens
        assert "[NAME_TITLE_1]" in state.tokens
        assert "[NAME_FIRST_3]" in state.tokens
        assert "[NAME_LAST_2]" in state.tokens
        
        # Verify original values preserved
        assert state.tokens["[NAME_TITLE_1]"].original == "Dr."
        assert state.tokens["[NAME_FIRST_3]"].original == "Sarah"
        assert state.tokens["[NAME_LAST_2]"].original == "Johnson"
    ...
    def test_partial_name_usage(self):
        ...
        # Entity tracker assigns indices alphabetically: "Doe" -> 1, "Jane" -> 2
        assert "[NAME_LAST_1]" in state.tokens
        assert "[NAME_FIRST_2]" in state.tokens
        
        # Simulate LLM using the actual tokens
        protected.notes = "Dear [NAME_FIRST_2], thank you for your message. - [NAME_LAST_1]"
...
class TestPIIConfig:
    ...
    def test_config_with_complex_transformation(self):
        ...
        def transform_profile_to_summary(profile: UserProfile) -> UserSummary:
            ...
            summary = UserSummary(
                ...
                account_type="premium" if "premium" in profile.preferences.get("tier", "") else "standard",
    ...
    def test_config_mixed_hallucination_actions(self):
        ...
        config = PIIConfig(
            detector=detector,
            on_hallucination=mixed_handler
        )
